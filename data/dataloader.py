    ...
    def _neg_sampling_collate_fn(self, batch):
        ...
        while True:
            ...
            for i in range(batch_size):
                mask[i] = np.isin(item_neg_idxs[i],
                                  self.interaction_sampler.dataset.user_sampling_matrix[user_idxs[i]].indices,
                                  assume_unique=True)
            to_resample = mask.sum()
...
class NegativeSamplingDataLoader(DataLoader):
    """
    NegativeSamplingDataLoader that performs negative sampling.
    """

    def __init__(self, dataset: TrainRecDataset, *args, **kwargs):
        self._dataset = dataset

        self.n_negative_samples = dataset.n_negative_samples
        self.negative_sampling_strategy = dataset.negative_sampling_strategy

        if self.negative_sampling_strategy not in {'uniform_recbole'}:
            raise ValueError(f'sampling strategy {self.negative_sampling_strategy} not '
                             f'supported for dataloader sampling!')

        super().__init__(dataset, *args, collate_fn=self._neg_sampling_collate_fn, **kwargs)

    def sampling(self, n_items):
        return np.random.choice(self._dataset.items_in_split, size=n_items, replace=True)
    ...
    def _neg_sampling_collate_fn(self, batch):
        ...
        """
        with record_function("sampling_collate_fn"):
            user_indices = np.array([x[0] for x in batch]).astype(np.int64)
            item_pos_indices = np.array([x[1] for x in batch])
            n_positive = item_pos_indices.shape[-1] if len(item_pos_indices.shape) > 1 else 1
            positive_user_items = [self._dataset.sampling_row_indices[idx] for idx in user_indices]

            user_indices_to_id_map = {idx: i for i, idx in enumerate(user_indices)}

            total_samples = len(user_indices) * self.n_negative_samples

            value_ids = np.zeros(total_samples, dtype=np.int64)
            check_list = np.arange(total_samples)
            sampling_user_indices = np.tile(user_indices, self.n_negative_samples)

            while len(check_list) > 0:
                # implementation following RecBole
                # see https://github.com/RUCAIBox/RecBole/blob/master/recbole/sampler/sampler.py
                value_ids[check_list] = self.sampling(len(check_list))
                check_list = np.array([
                    i
                    for i, user_index, v in zip(check_list,
                                                sampling_user_indices[check_list],
                                                value_ids[check_list]
                                                )
                    if v in positive_user_items[user_indices_to_id_map[user_index]]
                ])
            item_neg_indices = value_ids.reshape(self.n_negative_samples, -1).T

            # collect data to return it
            item_indices = np.column_stack([item_pos_indices, item_neg_indices]).astype(np.int64)
            labels = np.zeros_like(item_indices, dtype=float)
            labels[:, :n_positive] = 1.
            return torch.from_numpy(user_indices), torch.from_numpy(item_indices), torch.from_numpy(labels)
