...
if not os.path.exists(f'./raw_dataset/{category_short}_verified') or force_download:
    if force_download and os.path.exists(f'./raw_dataset/{category_short}_verified/'):
        shutil.rmtree(f'./raw_dataset/{category_short}_verified/')
    download_amazon2024_dataset('./', category_official=category_official, category_short=category_short,
                                verified_purchase=True)

print('Filtering on verified...')
filter_on_verified(category_official=category_official, category_short=category_short)

if os.path.exists(f'./processed_dataset/{category_short}_verified/'):
    shutil.rmtree(f'./processed_dataset/{category_short}_verified/')
os.mkdir(f'./processed_dataset/{category_short}_verified/')

ratings_path = f'./raw_dataset/{category_short}_verified/{category_official}.csv'
log_filt_data_file = open(os.path.join(f'./processed_dataset/{category_short}_verified', LOG_FILT_DATA_PATH), 'w+')
...
filtered_items = filter_on_meta(category_official=category_official, category_short=f'{category_short}_verified',
                                crawl_images=False)
...
print_and_log(log_filt_data_file, len(lhs), lhs.user.nunique(), lhs.item.nunique(),
              f'{k_core}-core filtering')
lhs.to_csv(f'./processed_dataset/{category_short}_verified/listening_history.csv', index=False)
"""
...
lhs.to_csv(f'./{category_short}_verified/processed_dataset/listening_history.csv', index=False)
train_data.to_csv(f'./{category_short}_verified/processed_dataset/listening_history_train.csv', index=False)
val_data.to_csv(f'./{category_short}_verified/processed_dataset/listening_history_val.csv', index=False)
test_data.to_csv(f'./{category_short}_verified/processed_dataset/listening_history_test.csv', index=False)

user_idxs.to_csv(f'./{category_short}_verified/processed_dataset/user_idxs.csv', index=False)
item_idxs.to_csv(f'./{category_short}_verified/processed_dataset/item_idxs.csv', index=False)
"""
