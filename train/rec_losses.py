...
from data.config_classes import ExperimentConfig, TrainDatasetConfig
...
class RecommenderSystemLoss(ABC):
    def __init__(self, n_items: int = None, aggregator: str = 'mean', train_neg_strategy: str = 'uniform',
        ...
        assert train_neg_strategy in ['uniform', 'uniform_recbole'], "Type of Negative Strategy not currently supported"
    ...
    @staticmethod
    def build_from_conf(conf: ExperimentConfig, dataset):
        train_conf = TrainDatasetConfig.from_dict_ext(conf.dataset, split='train')

        rec_loss_class = RecommenderSystemLossesEnum[conf.learn.rec_loss]
        return rec_loss_class.value(
            n_items=dataset.n_items,
            aggregator=conf.learn.loss_aggregator,
            train_neg_strategy=train_conf.negative_sampling_strategy,
            neg_train=train_conf.n_negative_samples
        )
